...
export default function Students() {
  ...
  async function handleAddPayment() {
    ...
    const { error: stuErr } = await supabase.from('students')
      .update({
        total_credits: student.total_credits + added,
        remaining_credits: student.remaining_credits + added,
      })
    ...
    setShowPayment(null)
    setPayForm({ credits: '', date: new Date().toISOString().split('T')[0] })
    fetchStudents()
  }
  ...
  async function exportCSV() {
    const rows = [
      ...
      ...students.map(s => [s.name, s.parent_name, s.email, s.total_credits, s.remaining_credits, s.created_at ? s.created_at.split('T')[0] : ''])
    ]
  ...
  return (
    <>
      ...
      <div className="page-wrap">
        <div className="page-head">
          ...
          <div className="head-btns">
            ...
            {isAdmin && (
              <Button variant="primary" size="sm" onClick={() => { setForm(emptyForm); setShowAdd(true) }}>
                + Add
              </Button>
            )}
          </div>
        ...
        <div className="controls">
          ...
          <button
            className={filterLow ? 'filter-btn active' : 'filter-btn'}
        ...
        {loading ? (
          ...
          <div className="list">
            {filtered.map(s => {
              ...
              return (
                <Card key={s.id} className={low ? 'student-row low' : 'student-row'}>
                    ...
                    {isAdmin ? (
                      <>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setShowEdit(s)
                            setForm({ name: s.name, parent_name: s.parent_name || '', email: s.email || '', total_credits: s.total_credits })
                          }}
                        >
                          Edit
                        </Button>
      ...
      <Modal open={!!showPayment} onClose={() => setShowPayment(null)} title={'Add Credits — ' + (showPayment ? showPayment.name : '')}>
        <div className="form-stack">
          <div className="current-credits">
            Current: <strong>{showPayment ? showPayment.remaining_credits : 0} credits remaining</strong>
          </div>
